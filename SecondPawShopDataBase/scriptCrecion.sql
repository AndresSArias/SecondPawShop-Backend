...
DROP VIEW IF EXISTS VISTACARRO;
CREATE VIEW VISTACARRO AS
SELECT P.IMAGEN, P.NOMBRE, V.CANTIDADACOMPRAR, P.PRECIO, V.PRECIOTOTAL, V.IDUSUARIOPROPETARIO, V.IDUSUARIOCOMPRADOR 
FROM PRODUCTO P, VENTA V
WHERE V.IDUSUARIOPROPETARIO = P.IDUSUARIOFK AND V.NOMBREPRODUCTO = P.NOMBRE
AND V.ESTADO = "CARRO";

DROP VIEW IF EXISTS VISTAIDCARRO;
CREATE VIEW VISTAIDCARRO AS
SELECT DISTINCT IDVENTA,  IDUSUARIOCOMPRADOR FROM VENTA WHERE ESTADO = "CARRO" ORDER BY IDVENTA DESC;

DROP VIEW IF EXISTS VISTAIDCOMPRADO;
CREATE VIEW VISTAIDCOMPRADO AS
SELECT DISTINCT IDVENTA,  IDUSUARIOCOMPRADOR FROM VENTA WHERE ESTADO = "COMPRADO" ORDER BY IDVENTA DESC;
